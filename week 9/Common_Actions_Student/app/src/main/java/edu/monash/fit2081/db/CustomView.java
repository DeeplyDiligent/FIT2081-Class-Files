...
public class CustomView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < numberShapes; i++) {
            ...
            else if (shapes[i].getShapeType().equals("SLine"))
                canvas.drawLine(shapes[i].getX(), shapes[i].getY(), shapes[i].getX() + shapes[i].getWidth(), shapes[i].getY() + shapes[i].getHeight(), paint);
            else if (shapes[i].getShapeType().equals("Oval"))
                canvas.drawOval(shapes[i].getX(), shapes[i].getY(), shapes[i].getX() + shapes[i].getWidth(), shapes[i].getY() + shapes[i].getHeight(), paint);
        }
    }
}